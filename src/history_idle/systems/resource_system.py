from ..models.resource import ResourceStorage
...
class ResourceProductionSystem:
    ...
    def calculate_resource_production(
        ...
        for allocation in population.allocations:
            if (
                allocation.task == WorkforceTask.RESOURCE_EXTRACTION
                and allocation.resource_id
            ):
                ...
                production[resource_id] = (
                    production.get(resource_id, 0.0) + total_production
                )

        return production

    def apply_production(
        self, resource_storage: ResourceStorage, production: dict[str, float]
    ) -> dict[str, float]:
        actual_added = {}
    ...
    def update(
        ...
        production = self.calculate_resource_production(
            resource_storage, population, delta_time
        )
...
class ResourceConsumptionSystem:
    ...
    def apply_consumption(
        self, resource_storage: ResourceStorage, consumption: dict[str, float]
    ) -> dict[str, float]:
        actual_consumed = {}
